						...
						.setPositiveButton("Yes",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
										...
										/*
										 * Getting the checked items from the
										 * listview
										 */
										SparseBooleanArray checkedItemPositions = getListView()
												.getCheckedItemPositions();
										int itemCount = getListView()
												.getCount();
										int result = -1;
										for (int i = itemCount; i >= 0; i--) {
											if (checkedItemPositions.get(i)) {
												adapter.remove(lists.get(i)
														.getName());
												database.open();
												NoteRecord r = database
														.GetNoteRecord(lists
																.get(i).getid());
												result = database
														.DeleteRecordFromNoteTbl(lists
																.get(i).getid());
												if (r != null) {
													database.DeleteThemeRecord(r.THEMEID);
													if (r.ISIMPORTANT == 1) {
														ImportantRecord ir = database
																.GetImportantRecord(r.ID);
														Intent k = new Intent(
																"com.ham.activity.AlertActivity");
